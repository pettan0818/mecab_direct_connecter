...
def lang_parser(text: str) -> NamedTuple:
    """determine text part lang and split.

    >>> lang_parser("この車は、日本では4WDと言われているが、その実態はFDである。")
    lang_info(lang=['J', 'E', 'J', 'E', 'J'], raw_text=['この車は、日本では', '4WD', 'と言われているが、その実態は', 'FD', 'である。'])
    >>> lang_parser("4WDはいいぞ。(https://ja.wikipedia.org/wiki/%E5%9B%9B%E8%BC%AA%E9%A7%86%E5%8B%95)")
